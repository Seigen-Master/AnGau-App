// Edge Function: Clock In/Out
// Replaces Firebase Cloud Function: caregiverClockInOut
...
serve(async (req) => {
  ...
  try {
    ...
    if (schedule.caregiver_id !== userData.id) {
      return new Response(
        JSON.stringify({ error: 'You are not assigned to this schedule' }),
        { 
          status: 403, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    if (action === 'clockIn') {
      ...
      return new Response(
        JSON.stringify({ result: 'Successfully clocked in' }),
        { 
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
